...
const json=async(url:string,init?:RequestInit)=>{const r=await fetch(url,{...init,signal:AbortSignal.timeout(8000)});if(!r.ok)throw new Error(`API ${r.status}`);return r.json() as Promise<any>};
...
async function verifyBinance(reference:string,get:Gettings):Promise<Verification>{
  const key=await get("binance_api_key"),secret=await get("binance_secret_key");
  ...
  try{
    const now=Date.now(),params=new URLSearchParams({timestamp:String(now),startTime:String(now-hour),endTime:String(now),limit:"100",recvWindow:"10000"});
    params.set("signature",createHmac("sha256",secret).update(params.toString()).digest("hex"));
    const history=await json(`https://api.binance.com/sapi/v1/pay/transactions?${params}`,{headers:{"X-MBX-APIKEY":key}});
  ...
  }catch{/* Some accounts use Merchant Pay credentials; try the merchant query below. */}
  try{
  ...
  }catch{return{state:"api_error",message:"Binance could not verify this transaction. Check API permissions or transaction ID."}}
}
